...
# image6 = cv2.morphologyEx(image2,cv2.MORPH_OPEN,kernal)
...
for i in range(len(a)):
    if a[i] > 100 and start == 0:
        a_Start.append(i)
        index1 = a_Start[0]
        start = 1
    if a[i] < 6  and start == 1:
        a_End.append(i)
        if (a_End[0] - index1) < 50:
            a_End.clear()
            start = 1
            continue
        start = 0
...
for i in range(len(a_Start)):
    ...
    for j in range(len(b)):
        if b[j] >= 10 and bstart ==0:
            b_Start =j
            bstart = 1
            bend=0
        if b[j] < 10 and bstart == 1:
            b_End =j
            bstart = 0
            bend=1
        if bend == 1:
